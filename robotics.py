...
def to_str_time(time_in_second):
    hours = time_in_second // 3600
    minutes = (time_in_second % 3600) // 60
    seconds = (time_in_second % 3600) % 60
...
time_in_second = to_seconds(starting_time_parts[0], starting_time_parts[1], starting_time_parts[2])

products = read_products()

while products:
    time_in_second = (time_in_second + 1) % (24 * 60 * 60)
    ...
    for robot_name in available_robots:
        if robot_name not in processing_robots:
            print(f'{robot_name} - {current_product} [{to_str_time(time_in_second)}]')
